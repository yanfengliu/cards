...
`src/run/` is the layer the diagram above leaves out. It depends on `engine` and, by design, on `content` — `src/run/content.ts` is that binding — and `ui` and `sim` depend on it: a run is a pure function of its seed, its class and its unlock set, played through the same engine a single fight is.
...
Both halves read a reference in every spelling it can take, and for one round neither read the spelling this repo uses. A global counts as referenced when it is written bare (`document`), as a member of the global object (`globalThis.document`, `window.x`, `self.x`) or as a string index on it (`globalThis['document']`) — the TypeScript checker resolves all three to the same symbol. A storage global counts by **name** wherever it is read, whatever the base, so aliasing the global object (`const g = globalThis; g.localStorage`) does not get past it; the DOM half cannot do that, because it asks the checker and the checker resolves `someElement.title` into `lib.dom` just as readily, so it tests the base and misses an aliased DOM global. Both bounds are in `tools/gates/boundaries.ts`'s header.

The correction is worth recording because of how it hid. The member name of a property access was exempt from both halves, and **every** storage access in the repo is written `globalThis.localStorage` — `src/ui/profile.ts`, `src/ui/runapp.ts`, `tools/ui-probe/run.ts`, with not one bare `localStorage` under `src/`. So `globalThis.localStorage?.getItem(…)` added to `src/run/run.ts` passed `gate:boundaries`, `tsc` and `gate:banned-apis`, while the gate printed that the file referenced *none* of them. The gate's own probe emitted bare identifiers, so the instrument check passed on a shape the code never uses, and the recorded red-proof used the bare form too: detector, self-check and evidence all agreed with each other and none of them touched real code. Each spelling now has its own probe line and must be caught **on that line**, so one shape cannot answer for the rest.
...
- **Inside one unit, traits fire in the source order of the `if` blocks in `triggersFor`.** There is no priority number on a trait, so moving a block moves the rule. **Two shipped traits answer `afterActed`: Relay, then Chorus** — a body carrying both emits `[right +2 Relay, left +2 Chorus, right +2 Chorus]`, and swapping the blocks changes that stream. It is reachable without a seam: `si_relay` grants Relay to a card that does not print it, and the two Chorus cards do not print Relay, so a run hands the engine such a body. Gated twice in `test/resolver-order.test.ts` — a fixture pinning that stream, and an AST check that reads the trait names out of the `afterActed` branch and pins them *in order*, so a third trait there goes red when it is written. The AST half replaces a tripwire that this line used to describe and that did not work: it asserted a hand-built body carrying `['relay', 'wake']` produced one effect per event, which a *third* trait keyed to `afterActed` does not appear on, so Chorus landed in that branch with the whole suite green. A gate built from a hand-written list can only see what somebody remembered to write into it. Board index is gated there directly, and it needs the `extra` seam: every shipped trigger is keyed to one `uid`, so no event matches two units.
...
**A race is not in `hashFight`'s canonical form, and that is deliberate.** A tribe is a function of `cardId`, which the canonical form already carries, and unlike a piece of equipment's two numbers it is identity rather than a tuned value: a rebalanced card keeps its id and a redesigned one gets a new one, so changing a card's race mints a new id. Every hash recorded before tribes existed still reproduces. **That last rule is the whole load-bearing part, and it is gated by a data pin** — `RACE_OF` in `test/tribes.test.ts` writes out every shipped card's race, and a card whose race moves has to move that line too. Without it, changing `u_songkeeper` from elf to human with the id kept passed `npm run gates`, `verify` and `verify:run` while changing what Chorus does in every fight that card appears in. The pin is written out rather than derived from the pool, for the reason `verify:run`'s sigil ledger records: a check built from the thing it checks proves only that the code agrees with itself.

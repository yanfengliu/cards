...
What that costs is bounded by one rule: **the resolver announces every change it makes and nothing it does not.** A swing that finds no legal target changes nothing, so it emits no event — an attack's `fizzled` could only ever be printed under the announcement of the death that ended the fight, and that is what a player read as "the Warchief dies", then "has no legal target". A burn that does land is still announced, dead hero or not, because the board really did change. A *spell* cast into an empty line still fizzles: energy was spent on it.

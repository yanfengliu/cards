...
**Still open at the end of round 1.** No independent review of those changes. Closed by round 2 below.

## Review round 2 — a second independent review, closed

The second review **passed** the things that mattered most, and none was redone: `canonicalUnlockSet` closes the live-vs-replay hash split at every entry point (`startRun` is the only `RunState` constructor, and the reviewer verified that), `migrateRunLog`'s refusal is reachable and well worded, `storageAllowed` survives a browser attack end to end, and `ATTRIB` genuinely reproduces the old runner's failure on both reporters.

Eight findings, every one confirmed by reproduction before it was touched, every one fixed and gated. Base `d59a8cb` with `main` at `308d91c` merged in — that merge is how `gate:work-plans` and the list-reading rule reach this branch, and it resolved five conflicts by hand, including `runArm` losing its `name` parameter on main while gaining `unlocked` here.

**G1 — the storage gate was aimed at an idiom this repo never writes.** `tools/gates/boundaries.ts` guarded both hidden-input halves with `!isNameSlot(node)`, and `isNameSlot` returns true for the `.name` of a property access. So `globalThis.localStorage` was skipped entirely — and that is how **every** storage access in this codebase is written (`profile.ts`, `runapp.ts`, `ui-probe/run.ts`; not one bare `localStorage` under `src/`). `globalThis.localStorage?.getItem(…)` in `src/run/run.ts` passed `gate:boundaries`, `tsc` and `gate:banned-apis` together, while the gate printed that the file referenced *none* of them. The blindness was self-confirming: the gate's own probe emitted bare identifiers and the recorded proof N13 used the bare form too, so detector, self-check and evidence all agreed with each other and none touched real code. Fixed in all three: the detector reads a global bare, through the global object and through a string index, and reads a storage global by name wherever it is read so an alias cannot get past it; the probe carries fifteen spellings, each bound to **its own line**; the record carries the idiom. Red as M1–M4, and M5 — reintroducing the exemption exits 2 and names the seven shapes lost. `AGENTS.md` and `ARCHITECTURE.md` corrected, both of which asserted a reach the gate did not have.

**G2 — "no persistent power" had moved its boundary and the escape moved with it.** `RunState` copies four of `RunContent`'s eighteen fields; the `startRun` walk skipped `content` wholesale; `unlockedContent`'s walk stops at its own function; `contentForClass` sits between. `restHealFraction + 0.25` at a fresh profile and `shopBasePrice: 0` with `maxRounds + 5` were both **273/273 green**, and both are literally what the walk's own message forbids. The walk covers `run.content` now, excepting only `rewards`, `sigils` and `classes[].rewards` — what the sibling walk already enumerates. Red as M6 and M7.

**G3 — the runner's two self-probes were both negative controls.** `SELF` and `ATTRIB` each assert the runner *refuses* to credit something; neither asserts it can ever say RED. Blanking `failingLines()` left both reporting "unconfirmed, as they must", every mutation unconfirmed, and the process exiting **0** — a fully blinded runner passing its own guard. There are three now: `POSITIVE` is `ATTRIB`'s mutation under the name of a test it *does* break, required to come back RED. Re-blinded, it is the only probe that fails and the runner exits 2. `docs/policies/local-rules.md` says three, and why the third is a different kind.

**G4 — the wiring gate was bound to one syntactic idiom, and restated its own subject.** `wiringOf` knew `dom.<key>.innerHTML = …` and nothing else, so routing a write through a two-line helper and removing the listener passed 2/2 with a dead button — and `sigils.ts` already exports `attachHtml`. It now covers `innerHTML`/`outerHTML` assignment, the DOM insert calls, a local alias, and a panel handed to any helper (loose in the safe direction, said so in the header). The second half is sharper: it restated `src/ui/runapp.ts` in the test body while the escaper half of the same file walked the filesystem — the rule that landed in `299fede`, four commits before. The subject is discovered now, and `src/ui/app.ts` is excused **by name with the reason**. Recorded with it: the obvious widening is a trap, because `runapp.ts` also has a `document.body` click listener and it is a narrow theme filter, not a delegation root. Red as M8 and M9.

**G5 — "no second escaper" was a single-quote substring search.** `.includes("'&amp;'")`, in a repo with no formatter. A double-quoted escaper was invisible, and the reviewer reintroduced the original `>` → `&quot;` defect that way. It reads string literals off the AST now and counts any HTML entity — exactly one file under `src/` writes one, so a partial escaper that never handles `&` is caught too. Red as M10 and M11.

**G6 — a stored run this build cannot read vanished with no on-screen word.** `parseSavedRun` returned `null` for "nothing stored" and "stored and refused" alike, so `opened` stayed `'fresh'` and `migrateRunLog`'s refusals were console-only; the blob also stayed in storage. `readSavedRun` keeps the reason, the notice bar says it, and the blob is cleared. Red as M12, and checked in a browser. The first draft of the sentence read *"The saved run written in format 99"* and passed its own test — the screenshot caught it.

**G7 — the pinned-session warning was about the wrong loss.** It said nothing *earned* is saved while `storageAllowed` was refusing the **run** as well: `?unlocks=all`, twenty minutes, a reload, gone. `pinnedNotice` is pure and exported — the reason `PICKABLE_CLASSES` is — and the notice bar says it on the first screen. Red as M13.

**G8 — the two smaller ones.** (a) `parseUnlockSet` ran before the unknown-format refusal, so a format-99 log with a malformed set never named its format; the format is read first (red as M14). (b) The `persist` threading was prose; `test/ui-markup.test.ts` already read that file as an AST, and now checks every call site, with the function list derived from "last parameter is `persist`" (red as M15).

**Checks at the closing revision.** `npm run gates` exit 0, **276 tests** (248 at `d59a8cb`, 273 at the merge). `node .probe/mutate.mjs`: **15 of 15 red**, each on a failing line and each absent from a measured green baseline, with all three self-probes correct. `npm run verify` and `npm run verify:run` differ from the merge revision only in `Elapsed` — every measured number unmoved. Browser, against a server whose own log named this worktree: the pinned warning and the refusal sentence on the notice bar and the collection screen, in both themes and at 375px.

**Still open.** No independent review of *round 2*. `AGENTS.md` and `ARCHITECTURE.md` are review-escalating and both are edited here; `src/run/run.ts` and `migrateRunLog` are inside the save/replay contract. The coordinator owns that call. `src/ui/app.ts` is outside the panel gate by a written excuse rather than by coverage — bringing it in needs a listener model this detector does not have.

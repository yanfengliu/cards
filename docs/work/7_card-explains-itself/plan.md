...
## Merged with unit 6 — 2026-09-07

Status: merged and reconciled on branch `worktree-agent-adcc5f0005b2166d6`; gates green; not reviewed, not merged to main.

This branch was cut from `b79abf9`, before unit 6 removed Ward, so integrating it meant reconciling rather than merging. `docs/work/6_trade-and-thin/plan.md`'s table of `src/ui/` and `src/render/` edits is what it was reconciled against, and the rule applied was **unit 7's structure, unit 6's behaviour**.

**Where the two disagreed.** Git conflicted on two declarations, both of which both branches had rewritten, and both took unit 7's side: `board.ts`'s `TRAIT_GLYPH` against `traitPipsOf`, and `app.ts`'s `TRAIT_RULE` against `traitRule`. Everything else in those two files merged cleanly and correctly — unit 6's `is-warded` toggles, `warded: false` ghost view and `'ward'` beat kind are gone, and its `retaliate` beat, log line and effect are present. Taking unit 7's side of the whole file instead leaves seven type errors; resolving only the declarations leaves three.

**What the compiler named**, which is the property `TRAIT_TERMS: Readonly<Record<Trait, Term>>` exists for: the `ward` entry in the glossary, the `ward` entry in `IconName`/`ICONS`, and two `warded: false` fixtures in `test/explain.test.ts`. Six further mentions were prose — two file headers, `TRIBE_TERMS.elf` naming the deleted Elf Warden, and three comments — and had to be found by reading.

**Added: the trade is explained.** Combat is mutual now, and that is a rule with no symbol on the card, so it goes in the summary line under the number strip: the unit's says attacking costs it, the hero's says a hero is the one exception, and the Power chip carries the whole rule including the 0-Power wall. It is not a rule row, and that is a measurement — a row costs 54px and takes the panel to 318px, which covers 112,062 px² of the hovered card's own line. The same sentence in the existing paragraph costs nothing.

**Re-measured**, `npm run probe:ui hover 7 even light` and `... dark` at 1440x900, with the copy in place:

| hovered | covers the opposite row | covers its own row |
|---|---|---|
| hand card (first, last) | 0 px² | 13,961 px² |
| enemy unit, ghost, player unit | 0 px² | 2,341 px² |
| enemy hero, player hero | 0 px² | 0 px² |
| enemy unit at 700x760 | 0 px² | 44,434 px² |

Identical to the numbers recorded above, in both themes. Hatching still off by default; the seven digests under `test/golden/heraldry/` are untouched.

**Looked at**, at native resolution, both themes: a player unit, a player hero, an enemy hero, a hand card, a ghost, the deuteranopia player row with hatching off and on, and frames 3-5 of `npm run probe:ui film 11 even light` — where the log reads "Human Champion acts / strikes Stone Troll for 3 (2 stopped by armour) / Stone Troll hits back for 3" and the Champion's Health disc drops 4 → 1 under a rising `−3`.

### Coverage lost to the merge

`npm run probe:ui hover` now prints `08-wide-player-unit: no element for #player-row .card`, and `narrow` reports "widest line reached: 3" at both `even` and `trivial`. With mutual damage the player's bodies die to the retaliation their own attacks draw, so the line never grows past three, and the compression-floor measurement recorded above — 19px of trait strip in a 44px card — has no state left to be photographed in. `test/explain.test.ts`'s arithmetic on `pipIconSize` still gates the rule. The picture is gone until the card pool is re-costed for a world where attacking has a price, which is open question 1 of `docs/work/6_trade-and-thin/plan.md`.

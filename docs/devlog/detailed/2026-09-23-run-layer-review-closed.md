...
**Correction, 2026-09-23, after the final acceptance review of `907c8e9`:** "so a race, a name or a hero's traits cannot move on the way in" was true of one setup, not of the fights a run plays. `fightSigilProblems` builds that setup from the finished run at act 0's first node, which is always an ordinary fight, and the test that reads real fights read only the hero's traits and name. The review changed what elite and boss fights are handed, and all seven gates stayed green on each change: card sigils stripped from every elite and boss fight (129 of 360 runs played differently), a sigilled card's race moved in boss fights (4 of 360), and hero-sigil Power and Armour dropped in boss fights (163 of 360). `watchFights` in `src/run/sigils.ts` now reads every fight a run plays as the engine holds it when the fight begins, through the same placement hook, and the ladder test and `verify:run` hold each one to the ledger. All three changes are red there at `0a1c8fd`; `docs/learning/gate-proofs.md` has the rows.
...
**Result:** Warm, five interleaved runs each, taken twice on a 32-thread machine. With other work holding it at about 20–35% CPU, `npm run gates` took a median 15.9s at `9b36329` and 12.4s at `fbbef39`, with the four new tests included. `verify:run`'s own command went from 6.2s to 3.8s, and each `npm run` start cost about 0.25s. At about 80% CPU earlier in the session the same pairs were 31.7s and 21.0s, 9.3s and 6.8s, and 0.45s.

**Reasoning:** The predecessor's figures, 2.6s of a 19.2s gate and 3.1s of an 8.4s `verify:run`, were taken on 2026-09-22. This session measured again, interleaved, and `tools/gates/chain.ts` and `src/sim/runmeasure.ts` now carry the 80%-load figures and say so. The same code through the old `&&` chain took 28.1s against 21.0s under that load, more than seven npm starts explain, and 13.3s against 12.4s at 20–35%, which they do explain. Load moves every one of these numbers, so compare pairs taken together and never numbers from different samples.

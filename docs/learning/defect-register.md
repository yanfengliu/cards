...
## 2026-09-23 — open: the fight screen's maximum Health is the Health the hero walked in with

**Status: open, not fixed.** Recorded by the final acceptance review of `main` at `907c8e9` as known issue 5, and deliberately left for a later round: it predates units 9 to 13 and is display-only.

**Symptom, as reported.** A Ranger run on seed 14 with nothing unlocked, played through the page with real clicks. At the fourth fight the HUD reads 121/180 with its bar about two thirds full, and the run's own line says "Your Ranger brings 121 of 180 Health into this fight". Beside it the fight's status line reads "you 121/121", and the hero card on the line shows 121 with a **full** bar. At that fight's end the banner reads "Your hero finished on 99 of 121 Health", the status line "99/121" and the card's bar about 82%, while the HUD reads 99/180. The second fight showed the same: "you 172/172" under a HUD of 172/180, then "139 of 172", then the reward shelf's correct "139 of 180". It compounds: each fight's maximum is where the previous fight left the hero, so the bar reads full at the start of every fight after one that did damage.

**Investigation.** Three places on the fight screen read the hero entity's `maxHealth`: the status line (`src/ui/app.ts:478`), the hero card's bar (`src/render/board.ts:489`) and the end banner (`src/ui/app.ts:674`). The HUD, the shelf and the run's own log line read the run's maximum, and they are right. Nothing in the engine reads `maxHealth`: it is set in `makeHero` and `makeUnit` (`src/engine/state.ts:308`, `:326`), copied by the snapshot (`:507`), and hashed (`src/engine/hash.ts:42`), and no rule compares against it. There is no heal verb, so no fight can raise Health toward it.

**Root cause.** `heroSpecFor` hands a fight the run's current Health (`src/run/nodes.ts:159`), and `makeHero` takes the Health it is handed as the maximum too (`src/engine/state.ts:308`). Nothing hands a fight the run's maximum. It has been so since unit 8's merge, `11688cb`, whose `src/run/run.ts:170` read `playerHero: { ...run.content.hero, health: run.hero.health }`, and unit 8's plan recorded it as a limitation. No design document chose it.

**Fix direction.** Fix it in the view. When a run hands the fight screen a fight, the status line, the hero card's bar and the end banner should read the run's maximum, as the HUD does, and a single fight with no run keeps the entity's. That moves no fight digest. The engine-side fix - a `HeroSpec` maximum that `makeHero` honours and `heroSpecFor` fills from the run - would move every run fight's digest, because `maxHealth` is in `hashFight`'s canonical form, so it needs a recorded decision first. Either way, the gate for it holds all three surfaces to the run over the fights `test/ui-won.test.ts` already plays, and the page probe reads the card's bar.

**What covers it today.** Nothing, by design: `test/ui-won.test.ts`'s header carves the fight's own maximum out of its claim, `tools/ui-probe/run.ts` compares only the Health a fight ended on, and `foughtSigilProblems` in `src/run/sigils.ts` does not hold the built hero's `maxHealth`, because holding it would pin this defect as correct.
...
**What is still not covered.** The unit gate cannot see whether `runapp.ts` wires these functions in: cutting the HUD from `heroNow`, or not handing it the finished fight, leaves it green (W8c, W12c). The probe sees both, but it needs Chrome and a server and is outside `npm run gates`, so only the probe rule in `docs/policies/local-rules.md` makes anyone run it. A statement about the hero in any shape other than the two the test parses is not read. And the end banner's "of" is the fight's maximum, which is the Health the hero walked in with: `heroSpecFor` hands a fight only the run's Health and the engine takes it as the maximum. So after a damaged fight it differs from the HUD's maximum. No design document chose that, and nothing holds that number to the run. It is more than the banner - the fight's status line and the hero card's bar read it too - and it is its own open entry above.

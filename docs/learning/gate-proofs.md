...
Warm wall time for `npm run gates`, five interleaved runs of each, on a 32-thread machine at about 80% CPU from other work: a median **31.7s** at `9b36329`, and **21.0s** at `1a72436` including the four new tests. The same code through the old `&&` chain took 28.1s. `verify:run`'s own command took a median 9.3s playing four arms and 6.8s playing one. Each `npm run` start costs about 0.45s: `npm run gate:work-plans` 0.82s against 0.36s run directly.

 ...
 *
 * Every noun in this game is invented. "Ward", "Wake", "Relay", "gules", "a
 * mullet" - none of them mean anything until something on screen says what they
 * mean, and until this file existed the only place any of them was written down
 * was `docs/design/game.md`, which a player will never open.
 *
...
export const STAT_TERMS = {
  power: {
    ...
    icon: 'power',
    line:
      'What it swings for. The target’s Armour comes off every hit, and damage past the ' +
      'target’s Health is wasted.',
  },
  health: {
    name: 'Health',
    icon: 'health',
...
export const TRAIT_TERMS: Readonly<Record<Trait, Term>> = {
  ...
  ward: {
    ...
    icon: 'ward',
    line:
      'After this acts, the unit immediately to its right cannot be struck this turn. ' +
      'A warded unit is not a legal target at all, so an attack with nothing else to hit fizzles.',
  },
};
...
export const TRIBE_TERMS: Readonly<Record<Tribe, Term>> = {
  ...
  elf: { name: 'Elf', icon: 'elf', line: 'Your line’s protectors — Wardens and Sentinels.' },

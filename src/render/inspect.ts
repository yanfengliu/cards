 ...
 * "Ward", "Wake" and "hue" meant is not a lapse of memory; it is the card
 * failing to say.
 *
...
export function explainCard(e: EntityView, card: CardView, ctx: InspectContext): string {
  ...
  const numbersLine =
    ctx.chance === 0 && e.alive
      ? `A Guard on this side is alive, so nothing behind it can be targeted at all. ${NUMBERS_SUMMARY}`
      : NUMBERS_SUMMARY;

// Data only, no logic. Fifteen hand-authored cards: eleven the player can
// place, four the enemy fields. Enough traits to make placement mean something
// and no more - this is a probe, not the card pool.
//
...
for (const c of [...PLAYER_CARDS, ...PLAYER_CARDS_NO_CASCADE, ...ENEMY_CARDS]) {
  if (BY_ID.has(c.id)) {
    throw new Error(`content: duplicate card id ${c.id}; ids are unique and stable forever`);
  }
  BY_ID.set(c.id, c);
}
